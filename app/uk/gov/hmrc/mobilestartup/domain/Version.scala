...
object VersionRange {
  ...
  def apply(range: String): VersionRange =
    range.replaceAll(" ", "") match {
      case ValidFixedVersion(v)             => VersionRange(Some(v), lowerBoundInclusive = true, Some(v), upperBoundInclusive = true)
      case ValidVersionRangeLeftOpen(v)     => VersionRange(None, lowerBoundInclusive = false, Some(v), upperBoundInclusive = range.endsWith("]"))
      case ValidVersionRangeRightOpen(v)    => VersionRange(Some(v), range.startsWith("["), None, upperBoundInclusive = false)
      case ValidVersionRangeBetween(v1, v2) => VersionRange(Some(v1), range.startsWith("["), Some(v2), range.endsWith("]"))
      case Qualifier(q) if q.length() > 1   => VersionRange(None, lowerBoundInclusive = false, None, upperBoundInclusive = false, Some(q))
...
object Version {
  ...
  def toVer(v: String): (Int, Int, Int) = {
    ...
    if (elem.forall(x => isAllDigits(x)) &&
        ...
        !v.startsWith(".") &&
        !v.endsWith(".") &&
        !v.contains("..")) (elem.lift(0).getOrElse("0").toInt, elem.lift(1).getOrElse("0").toInt, elem.lift(2).getOrElse("0").toInt)
    else (0, 0, 0)
  }
...
case class Version(major: Int, minor: Int, revision: Int, buildOrQualifier: Option[Either[Long, String]] = None) extends Comparable[Version] {
  ...
  lazy val boqFormatted: Option[String] = buildOrQualifier.map { boqE =>
    boqE match {
